...
/**
 * QR コードを、角の丸い位置検出パターンと丸いドットの SVG パスにする。1 モジュールを 1 単位とし、周囲の余白は含めない。
 * 位置検出パターンは線ではなく塗りの輪で描き、どの描画系でも同じ太さにする。ドットは隙間が広いと読み取りにくくなるため、ほぼモジュールいっぱいの半径にする。
 */
export function qrPath(text: string) {
  const { size, modules } = encodeQr(text, 'M');
  const corners = [[0, 0], [size - 7, 0], [0, size - 7]];
  // 位置検出パターンは 3 つの角の 7×7 に決まっている。そこは下でまとめて描く。
