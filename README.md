...
- **カーソル**: lucide の線画に倣う(24 の格子、線幅 2、角丸)。矢印は mouse-pointer-2、リンクの上は sprout(押している間は少し縮む)、文字の上は text-cursor。桃色の線に白い縁取り。SVG を CSS の `cursor` に埋め込み(JS なし)、ホバーできる端末だけ。形を変えるときは `tools/cursors.py` を編集して実行する。
